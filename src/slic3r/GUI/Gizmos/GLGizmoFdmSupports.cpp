...
bool GLGizmoFdmSupports::on_init()
{
    ...
    m_desc["enforce_by_angle"] = _L("Enforce by angle");
...
void GLGizmoFdmSupports::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    const float autoset_slider_left = m_imgui->calc_text_size(m_desc.at("enforce_by_angle")).x + m_imgui->scaled(1.f);
    ...
    m_imgui->disabled_begin(m_setting_angle);
    ...
    if (m_setting_angle) {
        if (m_imgui->button(m_desc["enforce_by_angle"])) {
            select_facets_by_angle(m_angle_threshold_deg, false);
            m_angle_threshold_deg = 0.f;
        }
    } else
        m_imgui->text(m_desc["enforce_by_angle"] + ":");
    ...
    m_setting_angle = (m_angle_threshold_deg != 0.f);
